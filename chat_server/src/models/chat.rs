...
impl AppState {
    ...
    pub async fn update_chat_by_id(&self, id: u64, input: UpdateChat) -> Result<Chat, AppError> {
        ...
        let sql = match (&input.name, &input.chat_type) {
            (Some(_), None) =>
                r#"UPDATE chats SET name = $2 WHERE id = $1 RETURNING id, ws_id, name, type, members, created_at"#.to_string(),
            (None, Some(_)) =>
                r#"UPDATE chats SET type = $2 WHERE id = $1 RETURNING id, ws_id, name, type, members, created_at"#.to_string(),
            (Some(_), Some(_)) =>
                r#"UPDATE chats SET name = $2, type = $3 WHERE id = $1 RETURNING id, ws_id, name, type, members, created_at"#.to_string(),
            _ => {
                return Err(AppError::UpdateChatError(
                    "Invalid update fields".to_string(),
                ))
            }
        };

        let chat = sqlx::query_as(&sql)
            .bind(id as i64)
            .bind(input.name)
